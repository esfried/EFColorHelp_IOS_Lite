...
struct TriadicPortraitView: View {
...
struct TriadicLandscapeView: View {
    @EnvironmentObject var appModel: AppModel
    ...
    var body: some View {
        ...
        GeometryReader { m in
            VStack (spacing:0) {
                ...
                GeometryReader { m1 in
                    HStack(spacing:0) {
                        ChromaticCircle(isSplashScreen: false,wheelType: .triadic,showSelection: true,showPicker: false)
...
struct TriadicView_Previews: PreviewProvider {
    static var previews: some View {
       /* Landscape {
            TriadicView(orientation: .landscapeRight)
        ...
        Portrait {*/
            TriadicView(orientation: .portrait)
                .environmentObject(AppModel())
     //   }
    }
}
